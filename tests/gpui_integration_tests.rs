...
fn test_channel_communication() {
    ...
    let (view_cmd_tx, view_cmd_rx) =
        flume::unbounded::<personal_agent::presentation::ViewCommand>();
...
fn test_streaming_state_transitions() {
    ...
    assert!(matches!(
        state.streaming,
        StreamingState::Streaming { done: false, .. }
    ));
    ...
    assert!(matches!(
        state.streaming,
        StreamingState::Streaming { done: true, .. }
    ));
