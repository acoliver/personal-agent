...
async fn test_eventbus_round_trip() {
    ...
    assert_eq!(
        result.unwrap(),
        1,
        "Event should be received by 1 subscriber"
    );
    ...
    assert_eq!(
        received.unwrap(),
        event,
        "Received event should match published event"
    );
}
...
async fn test_conversation_service_crud() {
    ...
    assert!(
        load_result.is_err(),
        "Loading deleted conversation should fail"
    );
}
...
async fn test_secrets_service_crud() {
    ...
    // Test 2: Get secret
    let value = service.get("test_key").await.expect("Failed to get secret");
    ...
    // Test 4: List keys
    let keys = service.list_keys().await.expect("Failed to list keys");
...
async fn test_app_settings_service() {
    ...
    assert_eq!(
        retrieved_profile_id2,
        Some(profile_id),
        "Settings should persist across instances"
    );
    assert_eq!(
        retrieved_conversation_id2,
