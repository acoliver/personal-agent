...
async fn setup_test_environment() -> (broadcast::Sender<AppEvent>, mpsc::Receiver<ViewCommand>) {
    ...
    let profile_service: Arc<dyn ProfileService> =
        Arc::new(ProfileServiceImpl::new(profiles_dir).expect("ProfileService"));

    let app_settings_service: Arc<dyn AppSettingsService> =
        Arc::new(AppSettingsServiceImpl::new(settings_file).expect("AppSettingsService"));
...
async fn test_mcp_starting_event() {
    ...
    // Emit Starting event
    event_tx
        .send(AppEvent::Mcp(McpEvent::Starting {
            id: mcp_id,
    ...
    let has_status_change = commands
        .iter()
        .any(|cmd| matches!(cmd, ViewCommand::McpStatusChanged { .. }));

    assert!(
        has_status_change || commands.is_empty(),
...
async fn test_mcp_started_event() {
    ...
    event_tx
        .send(AppEvent::Mcp(McpEvent::Started {
            id: mcp_id,
            name: "test-mcp".to_string(),
...
async fn test_mcp_stopped_event() {
    ...
    event_tx
        .send(AppEvent::Mcp(McpEvent::Stopped {
            id: mcp_id,
            name: "test-mcp".to_string(),
        }))
        .ok();
...
async fn test_mcp_start_failed_event() {
    ...
    assert!(
        true,
        "Test completed - presenter processed StartFailed event"
    );
}
