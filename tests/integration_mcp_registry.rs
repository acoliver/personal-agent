...
async fn test_fetch_official_registry() {
    ...
    assert!(servers.is_ok(), "Should fetch official registry successfully");
    ...
    if let Some(first) = servers.first() {
        ...
        assert!(!first.server.description.is_empty(), "Server should have a description");
        assert!(!first.server.version.is_empty(), "Server should have a version");
        
        println!("Sample server: {} v{}", first.server.name, first.server.version);
...
async fn test_search_registry() {
    ...
    println!("Found {} servers matching 'github'", search_result.entries.len());
...
async fn test_empty_search() {
    ...
    let search_result = result.unwrap();
    println!("Found {} servers for non-existent query", search_result.entries.len());
}
