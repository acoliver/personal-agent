...
use personal_agent::ui_gpui::components::{
    AssistantBubble, Button, InputBar, Tab, TabBar, UserBubble,
};
...
fn test_assistant_bubble_streaming_state() {
    ...
    let complete_bubble = AssistantBubble::new("Complete response").streaming(false);
...
fn test_input_bar_streaming_state() {
    ...
    let idle_bar = InputBar::new().is_streaming(false);
