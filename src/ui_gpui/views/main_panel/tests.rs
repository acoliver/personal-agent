...
#[gpui::test]
async fn handle_command_forwards_tool_approval_request_to_chat(cx: &mut TestAppContext) {
    let (app_state, _user_rx, _first_id, _second_id, _selected_profile_id) = build_app_state();
    cx.set_global(app_state);
    let panel = cx.new(MainPanel::new);
    ...
    panel.update(cx, |panel: &mut MainPanel, cx| {
        ...
        chat_view.read_with(cx, |view, _| {
            ...
            assert_eq!(bubble.tool_name, "WriteFile");
            assert_eq!(bubble.tool_argument, "/tmp/example.txt");
            assert_eq!(
                bubble.state,
...
#[gpui::test]
async fn handle_command_forwards_tool_approval_resolved_to_chat(cx: &mut TestAppContext) {
    let (app_state, _user_rx, _first_id, _second_id, _selected_profile_id) = build_app_state();
    cx.set_global(app_state);
    let panel = cx.new(MainPanel::new);
    ...
    panel.update(cx, |panel: &mut MainPanel, cx| {
        ...
        panel.handle_command(
            ViewCommand::ToolApprovalRequest {
                ...
                tool_name: "WriteFile".to_string(),
                tool_argument: "/tmp/example2.txt".to_string(),
            },
            cx,
        );
        panel.handle_command(
