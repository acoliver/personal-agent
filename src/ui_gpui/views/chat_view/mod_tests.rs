...
// ── composer draft carry-over tests ──────────────────────────────────────
...
#[gpui::test]
async fn switching_to_a_conversation_without_a_draft_clears_the_composer(cx: &mut TestAppContext) {
    let view = cx.new(|cx| ChatView::new(ChatState::default(), cx));
    let mut visual_cx = cx.add_empty_window().clone();
...
#[gpui::test]
async fn returning_to_a_conversation_restores_its_own_draft(cx: &mut TestAppContext) {
    let view = cx.new(|cx| ChatView::new(ChatState::default(), cx));
    let mut visual_cx = cx.add_empty_window().clone();
