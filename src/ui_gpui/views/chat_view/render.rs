...
//!
//! Contains `render_chat_area`, message rendering helpers, and the root
//! `impl Render for ChatView`. These are the content-area methods below the
//! navigation bars. The composer row moved to `render_composer` when this
//! file reached the length CI rejects.
//!
    ...
    fn render_transcript_row(
        ...
        match row {
            ...
            TranscriptRow::Streaming => self
                ...
                .into_any_element(),
            TranscriptRow::QueuedSteering(index) => {
                Self::render_queued_steering(index, &self.state.queued_steering[index].text)
            }
        }
    }

    /// Render a steering message that is waiting for the turn's boundary.
    ///
    /// It sits on the user's side of the transcript because the user wrote
    /// it, but it is deliberately not a user bubble: an outline and a label
    /// say it has not been said to the model yet, so a queued steer is never
    /// mistaken for one the agent has already read.
    ///
    /// @plan PLAN-20260903-ISSUE222.P04
    /// @requirement REQ-222-003
    fn render_queued_steering(index: usize, text: &str) -> gpui::AnyElement {
        div()
            .id(SharedString::from(format!("queued-steering-{index}")))
            .debug_selector(move || format!("chat-queued-steering-{index}"))
            .w_full()
            .flex()
            .justify_end()
            .child(
