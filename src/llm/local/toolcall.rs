...
//!
//! Ported verbatim from the verified `PoC` (`experiments/localmodel-toy`), with
//! one addition: [`parse_call_block`] exposes the inside-of-a-block parser so
//! the streaming layer can emit a tool call the moment `</tool_call>` closes.
//!
...
impl RawToolCall {
    ...
    /// lookup; the model layer iterates all arguments when rebuilding JSON, so
...
/// Parses the inside of one `<tool_call>` block (the text between the markers).
///
/// The streaming layer calls this the moment a closing marker is seen; it must
/// not have to rescan the whole transcript.
///
/// # Errors
///
/// Returns [`ToolCallParseError`] when the block is structurally broken.
pub fn parse_call_block(body: &str) -> Result<RawToolCall, ToolCallParseError> {
    parse_function(body)
}
...
mod tests {
    ...
    #[test]
    fn parse_call_block_matches_parse_response_for_one_block() {
        let body = "\n<function=next_secure_prime>\n<parameter=after>\n1000000\n</parameter>\n</function>\n";
        let block = parse_call_block(body).expect("block parses");
        let whole = parse_response(&format!("{TOOL_CALL_OPEN}{body}{TOOL_CALL_CLOSE}"))
            .expect("whole parses");
        assert_eq!(block, whole.calls[0]);
    }
}
