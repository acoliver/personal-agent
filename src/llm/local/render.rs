...
//!
//! Ported from the verified `PoC` (`experiments/localmodel-toy`). Granite ships
//! its chat template inside the GGUF, but rendering it by hand keeps the exact
//! byte-level prompt visible, diffable, and free of any template-engine
//! dependency. The shapes mirror the embedded template:
//!
//! ```text
//! <|im_start|>system
//! ...tools block...<|im_end|>
...
/// One tool schema as the `<tools>` block needs it.
///
/// The `PoC` rendered the same three fields; the model layer maps serdes
/// `ToolDefinition`s into this shape so the renderer stays free of any
/// provider vocabulary.
...
pub struct ToolSpec {
    ...
    /// Compact single-line JSON Schema of the parameters.
...
impl ToolSpec {
    ...
    ///
    /// Field order matches the embedded template (`name`, `description`,
    /// `parameters`), which `serde_json`'s map ordering would not preserve.
    #[must_use]
    pub fn to_compact_json(&self) -> String {
        let name = serde_json::to_string(&self.name).unwrap_or_default();
        let description = serde_json::to_string(&self.description).unwrap_or_default();
        format!(
...
pub enum ChatTurn {
    ...
    ///
    /// Multi-turn agent history replays previous answers through this shape.
    Assistant {
    ...
    AssistantToolCalls {
        ...
        calls: Vec<crate::llm::local::toolcall::RawToolCall>,
    },
...
///
/// The shape matches what [`crate::llm::local::toolcall::parse_response`] reads
/// back: optional rationale prose, then one `<tool_call>` block per call, each
/// closed with a newline.
fn render_assistant_tool_calls(
    rationale: Option<&str>,
    calls: &[crate::llm::local::toolcall::RawToolCall],
) -> String {
...
fn render_tool_call(call: &crate::llm::local::toolcall::RawToolCall) -> String {
...
mod tests {
    use super::*;
    use crate::llm::local::toolcall::RawToolCall;

    fn prime_tool() -> ToolSpec {
        ToolSpec {
            name: "next_secure_prime".to_string(),
            description: "Return the smallest secure prime strictly greater than a lower bound. A secure prime, also called a safe prime, is a prime p for which (p-1)/2 is also prime.".to_string(),
            parameters_json: r#"{"type":"object","properties":{"after":{"type":"integer","description":"Exclusive lower bound. The result is strictly greater than this value."}},"required":["after"]}"#.to_string(),
        }
    }
    ...
    fn search_tool() -> ToolSpec {
        ToolSpec {
            ...
            parameters_json: r#"{"type":"object","properties":{"query":{"type":"string","description":"The search query."}},"required":["query"]}"#.to_string(),
        }
    }

    /// The compact JSON the two tools render as, spelled out so the golden
    /// tests fail loudly if `ToolSpec` serialisation ever changes.
    ...
    #[test]
    fn tool_compact_json_matches_the_template_dialect() {
        assert_eq!(prime_tool().to_compact_json(), PRIME_TOOL_JSON);
        assert_eq!(search_tool().to_compact_json(), SEARCH_TOOL_JSON);
    }

    #[test]
    fn system_plus_tools_plus_user_matches_the_template_exactly() {
        let tools = vec![prime_tool(), search_tool()];
        let turns = vec![
            ChatTurn::System {
    ...
    fn tool_response_content_round_trips_through_the_parser() {
        ...
        let turns = vec![
            ...
            ChatTurn::ToolResponses {
                results: vec!["april fools this doesn't search anything".to_string()],
            },
        ];
        let rendered = render(&turns, &tools, true);
        ...
        let response_part =
            "<tool_response>\napril fools this doesn't search anything\n</tool_response>";
        ...
        let parsed =
            crate::llm::local::toolcall::parse_response(replay).expect("assistant block parses");
