    ...
    conversation
        ...
        .map(|msg| {
            match msg.role {
                MessageRole::User => {
                    let part =
                        ModelRequestPart::UserPrompt(UserPromptPart::new(msg.content.clone()));
...
pub async fn send_message_stream(
    ...
    // Get API key from OS keychain (or empty string for local models)
    let api_key = match &profile.auth {
        AuthConfig::None => String::new(),
        AuthConfig::Keychain { label } => {
            crate::services::secure_store::api_keys::get(label.trim())
    ...
    let mapped_stream = stream.map(|event| match event {
        Ok(AgentStreamEvent::TextDelta { text }) => ChatStreamEvent::text(text),
        Ok(AgentStreamEvent::ThinkingDelta { text }) => ChatStreamEvent::thinking(text),
        Ok(AgentStreamEvent::RunComplete { .. }) => ChatStreamEvent::complete(None, None),
        Ok(AgentStreamEvent::Error { message }) => ChatStreamEvent::error(message, true),
        Err(e) => ChatStreamEvent::error(e.to_string(), false),
        _ => {
