...
use objc2_app_kit::{
    NSAppearanceCustomization, NSButton, NSControlStateValueOn, NSFont, NSLayoutConstraintOrientation,
...
use uuid::Uuid;
use super::model_selector::{SELECTED_MODEL_PROVIDER, SELECTED_MODEL_ID, SELECTED_MODEL_BASE_URL, SELECTED_MODEL_CONTEXT};
...
impl ProfileEditorDemoViewController {
    pub fn new(mtm: MainThreadMarker) -> Retained<Self> {
        let this = mtm.alloc::<Self>().set_ivars(ProfileEditorDemoIvars::default());
    ...
    fn build_top_bar(&self, mtm: MainThreadMarker) -> Retained<NSView> {
        ...
        let cancel_btn = unsafe {
            NSButton::buttonWithTitle_target_action(
                &NSString::from_str("Cancel"), Some(self), Some(sel!(cancelClicked:)), mtm,
            )
        };
        ...
        let save_btn = unsafe {
            NSButton::buttonWithTitle_target_action(
                &NSString::from_str("Save"), Some(self), Some(sel!(saveClicked:)), mtm,
            )
        };
    ...
    fn build_form_scroll(&self, mtm: MainThreadMarker) -> Retained<NSScrollView> {
        ...
        *self.ivars().name_input.borrow_mut() = Some(name_section.1);
        ...
        let mut providers = vec!["anthropic", "openai", "google", "groq", "mistral", "ollama", "custom"];
        ...
        *self.ivars().model_popup.borrow_mut() = Some(model_section.1);
        ...
        if preselected_provider.is_some() && preselected_model.is_some() {
            log_to_file(&format!("Profile editor configured with: {}:{}", 
    ...
    fn build_popup_field(&self, label: &str, items: &[&str], mtm: MainThreadMarker) -> (Retained<NSView>, Retained<NSPopUpButton>) {
        let container = NSStackView::new(mtm);
        container.setOrientation(NSUserInterfaceLayoutOrientation::Vertical);
        container.setSpacing(4.0);
        container.setTranslatesAutoresizingMaskIntoConstraints(false);
        ...
        let width = popup.widthAnchor().constraintGreaterThanOrEqualToConstant(350.0);
        width.setActive(true);
