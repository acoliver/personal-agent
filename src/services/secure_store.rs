...
fn macos_security_not_found(stderr: &[u8]) -> bool {
    ...
    stderr.contains("could not be found") || stderr.contains("The specified item could not be found")
}
...
fn macos_security_get_secret(key: &str) -> Result<Option<String>, SecureStoreError> {
    ...
    if output.status.success() {
        let value = String::from_utf8(output.stdout)
            .map_err(|e| SecureStoreError::Keychain(format!("Invalid UTF-8 from security CLI: {e}")))?;
...
pub mod api_keys {
    ...
    fn load_index() -> Vec<String> {
        if let Some(path) = api_key_index_path() {
    ...
    fn save_index(labels: &[String]) -> Result<(), SecureStoreError> {
        ...
        if let Some(parent) = path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| SecureStoreError::Keychain(format!("Failed to create api key index dir: {e}")))?;
        }
        fs::write(path, json)
