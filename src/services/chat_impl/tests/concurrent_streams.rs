...
async fn begin_stream_allows_two_different_conversations() {
    ...
    // Insert mock streams to simulate active streams for both conversations
...
async fn begin_stream_rejects_same_conversation_twice() {
    ...
    // First begin_stream should succeed
    service
        .begin_stream_for_test(conversation_id)
        .await
        .expect("first begin_stream should succeed");

    // Insert a mock stream to simulate the stream being active
    service.insert_mock_stream_for_test(conversation_id);

    // Second begin_stream for same conversation should fail
    let err = service
        .begin_stream_for_test(conversation_id)
        .await
...
async fn cancel_scopes_to_conversation() {
    ...
    // Insert mock streams for both conversations
    service.insert_mock_stream_for_test(conversation_a);
    service.insert_mock_stream_for_test(conversation_b);
...
async fn cancel_unknown_conversation_is_noop() {
    ...
    // Insert mock stream for known conversation only
    service.insert_mock_stream_for_test(known_conversation);
