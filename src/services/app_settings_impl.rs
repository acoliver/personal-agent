...
impl AppSettingsServiceImpl {
    /// # Errors
    ///
    /// Returns `ServiceError` if the settings directory cannot be created,
    /// the settings file cannot be read, or existing JSON cannot be parsed.
...
impl AppSettingsService for AppSettingsServiceImpl {
    ...
    async fn get_launch_at_login(&self) -> ServiceResult<Option<bool>> {
        let storage = self.load()?;
        Ok(storage.launch_at_login)
    }

    async fn set_launch_at_login(&self, enabled: bool) -> ServiceResult<()> {
        let mut storage = self.load()?;
        storage.launch_at_login = Some(enabled);
        self.save(&storage)?;
