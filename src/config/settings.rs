...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.global_hotkey, "Cmd+Shift+Space");
        assert_eq!(config.default_profile, None);
        assert_eq!(config.profiles.len(), 0);
        assert_eq!(config.context_management.trigger_threshold, 0.80);
    }
    ...
    fn test_add_profile() {
        ...
        assert_eq!(config.profiles[0].id, profile_id);
    }

    #[test]
    fn test_remove_profile() -> Result<()> {
        let mut config = Config::default();
        let profile = ModelProfile::default();
        let profile_id = profile.id;
    ...
    fn test_remove_nonexistent_profile() {
        let mut config = Config::default();
        let result = config.remove_profile(&Uuid::new_v4());
        assert!(result.is_err());
    }

    #[test]
    fn test_get_profile() -> Result<()> {
        let mut config = Config::default();
        let profile = ModelProfile::default();
        let profile_id = profile.id;

        config.add_profile(profile);
        let retrieved = config.get_profile(&profile_id)?;
    ...
    fn test_update_profile() -> Result<()> {
        ...
        let updated = config.get_profile(&profile_id)?;
    ...
    #[test]
    fn test_remove_default_profile_clears_default() -> Result<()> {
        let mut config = Config::default();
        let profile = ModelProfile::default();
        let profile_id = profile.id;
