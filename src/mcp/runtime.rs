...
mod tests {
        ...
        runtime.connections.insert(
            ...
            McpConnection {
                ...
                client: Arc::new(Mutex::new(McpClient::new(
                    serdes_ai::mcp::transport::MemoryTransport::new(),
                ))),
                tools: vec![],
            },
        );
    ...
    #[tokio::test]
    async fn test_call_tool_sets_error_status_on_timeout() {
        let mut runtime = create_test_runtime();
        let config = create_test_mcp_config(true);
        let mcp_id = config.id;

        let transport = serdes_ai::mcp::transport::MemoryTransport::new();
        ...
        assert!(result.is_err());
        assert!(matches!(
            runtime.status_manager.get_status(&mcp_id),
            McpStatus::Error(_)
        ));
    }
    ...
    #[tokio::test]
    async fn test_start_mcp_fails_fast_on_missing_required_package_args() {
        let mut runtime = create_test_runtime();
        let mut config = create_test_mcp_config(true);

        // Add required package_args metadata
        config.package_args = vec![crate::mcp::McpPackageArg {
            ...
            description: Some("Directories to allow access to".to_string()),
            required: true,
            default: None,
        }];

        // Don't provide the required value in config
