...
impl SecretsManager {
    ...
    pub fn store_api_key_named(&self, mcp_id: Uuid, var_name: &str, key: &str) -> Result<(), SecretsError> {
...
mod tests {
    ...
    fn test_store_and_load_named_api_key() {
        ...
        manager.store_api_key_named(mcp_id, "CLIENT_ID", key1).unwrap();
        manager.store_api_key_named(mcp_id, "CLIENT_SECRET", key2).unwrap();
    ...
    fn test_load_nonexistent_key() {
        ...
        assert!(matches!(result.unwrap_err(), SecretsError::SecretNotFound(_)));
    }
    ...
    fn test_delete_multiple_keys() {
        ...
        manager.store_api_key_named(mcp_id, "CLIENT_ID", "key2").unwrap();
        manager.store_api_key_named(mcp_id, "CLIENT_SECRET", "key3").unwrap();
    ...
    fn test_read_nonexistent_keyfile() {
        ...
        assert!(result.is_err());
        assert!(matches!(result.unwrap_err(), SecretsError::KeyfileNotFound(_)));
    }
