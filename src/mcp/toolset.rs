...
use crate::mcp::{McpConfig, McpPackageType, McpTransport, McpAuthType};
...
mod tests {
    use super::*;
    use crate::mcp::{McpSource, McpPackage, EnvVarConfig};
    ...
    async fn test_build_command_npm() {
        ...
        let (cmd, args) = build_command(&config);
        assert_eq!(cmd, "npx");
        assert_eq!(args, vec!["-y".to_string(), "@mcp/server-filesystem".to_string()]);
