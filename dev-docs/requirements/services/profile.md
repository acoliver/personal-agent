...
- ModelProfile is defined in `dev-docs/requirements/data-models.md` and owned by ProfileService.
- AuthMethod is UI-derived only (not stored).
...
|----|----------|----------------|
| PR-NT1 | create with empty name | VALIDATION_ERROR, not saved |
| PR-NT2 | create with no auth fields | VALIDATION_ERROR, not saved |
| PR-NT3 | update missing id | NOT_FOUND, not saved |
| PR-NT4 | delete last profile | CONFLICT, not deleted |

## End-to-End Flow (Profile Save)

1. validate(profile) returns field errors if any.
2. create/update writes config.json.
3. If write fails, return SERVICE_UNAVAILABLE and keep prior config.
